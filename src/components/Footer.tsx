
import React from 'react';
import { Link } from 'react-router-dom';
import { ArrowUp } from 'lucide-react';

const Footer = () => {
  const year = new Date().getFullYear();
  
  const scrollToTop = () => {
    window.scrollTo({
      top: 0,
      behavior: 'smooth'
    });
  };
  
  return (
    <footer className="relative overflow-hidden border-t border-white/10">
      {/* Background effect */}
      <div className="absolute inset-0 bg-noise-pattern opacity-5"></div>
      
      <div className="container mx-auto px-4 py-16">
        <div className="flex flex-col lg:flex-row items-start justify-between">
          {/* Logo and Description */}
          <div className="mb-12 lg:mb-0 max-w-sm">
            <Link to="/" className="inline-block mb-6">
              <div className="text-neon-pink font-bold text-2xl flex items-center">
                <span className="text-white">Game</span>
                <span className="text-gradient">Topia</span>
              </div>
            </Link>
            
            <p className="text-white/70 mb-8">
              Our platform brings together the latest and most exciting games, featuring both
              rising developers and established studios. Ready to explore?
            </p>
            
            <div className="flex items-center space-x-4">
              {['instagram', 'facebook', 'twitter', 'linkedin'].map((social, index) => (
                <a 
                  key={index}
                  href="#" 
                  className="flex items-center justify-center w-10 h-10 rounded-full bg-white/10 text-white/80 hover:bg-neon-pink/20 hover:text-white transition-colors duration-300"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                  </svg>
                </a>
              ))}
            </div>
          </div>
          
          {/* Footer Links */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 w-full lg:w-auto">
            {/* Company */}
            <div>
              <h4 className="text-xl font-bold mb-6">Company</h4>
              <ul className="space-y-4">
                {['Home', 'About us', 'Apps & Games', 'Discover'].map((item, index) => (
                  <li key={index}>
                    <a href="#" className="text-white/70 hover:text-white transition-colors duration-300">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
            
            {/* Support */}
            <div>
              <h4 className="text-xl font-bold mb-6">Support</h4>
              <ul className="space-y-4">
                {['Community', 'Learn', 'Contact', 'Blog'].map((item, index) => (
                  <li key={index}>
                    <a href="#" className="text-white/70 hover:text-white transition-colors duration-300">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
            
            {/* Legal */}
            <div>
              <h4 className="text-xl font-bold mb-6">Legal</h4>
              <ul className="space-y-4">
                {['Privacy Policy', 'Terms & use', 'Faq'].map((item, index) => (
                  <li key={index}>
                    <a href="#" className="text-white/70 hover:text-white transition-colors duration-300">
                      {item}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
        
        {/* Copyright */}
        <div className="pt-10 mt-10 border-t border-white/10 text-center">
          <p className="text-white/50">© All Right Reserved. {year}. GameTopia</p>
        </div>
        
        {/* Scroll to top button */}
        <div className="flex justify-end mt-6">
          <button 
            onClick={scrollToTop}
            aria-label="Scroll to top"
            className="flex items-center justify-center w-12 h-12 rounded-full bg-white/5 border border-white/10 text-white/70 hover:bg-neon-pink/20 hover:text-white transition-all duration-300 hover:scale-105 hover-glow"
          >
            <ArrowUp className="h-5 w-5" />
          </button>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
